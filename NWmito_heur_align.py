...
def needleman_wunsch(string1=ref_string, string2=query_string, match_score = 1, mismatch_score = -1, gap_penalty = default_gap_penalty):
	...
	def score(row_number, x_loc):
		...
		def calc_gaps():
			...
			k = 1
			while (x_loc + k) < row_width and row_number - k >= 0:
				gap_dict[scoring_matrix[row_number-k][x_loc+k] - gap_penalty(k)] = k
				k += 1
			j = 1
			while (x_loc - j) >= 0 and row_number - j >= 0:
				gap_dict[scoring_matrix[row_number-j][x_loc-j] - gap_penalty(j)] = -j
				j += 1
			return gap_dict
	...
	for row in range(shorter_len):
		for x_loc in range(row_width):
			scoring_matrix[row+1][x_loc] = score(row+1, x_loc)
